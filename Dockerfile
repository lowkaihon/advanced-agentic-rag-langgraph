...
# Prevent OpenMP/MKL deadlocks in containers (fixes HHEM local backend hanging)
...
# Pre-download HuggingFace models at build time (baked into image)
# HHEM: Local backend (default) for hallucination detection
# CrossEncoder: Reranking (always needed)
RUN python -c "\
from transformers import AutoModelForSequenceClassification, AutoTokenizer; \
from sentence_transformers import CrossEncoder; \
print('Downloading HHEM model for local backend...'); \
